...
document.addEventListener('WebComponentsReady', () => {
    ...
    if (/bundle/i.test(window.location)) {
        viewer.package = '/urdf';
    }
...
const updateAngles = () => {
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        viewer.setJointValue(jointName, THREE.MathUtils.lerp(-50, 50, ratio) * DEG2RAD);
    }
    // animate several robots
    /* if (viewer.urdf && viewer.urdf.includes("UR5")) {
        for (let i = 1; i <= 6; i++) {
            const jointName = `joint${i}`;
            const offset = i * Math.PI / 3;
            const ratio = Math.max(0, Math.sin(time + offset));
    ...
    else if (viewer.urdf && viewer.urdf.includes("T12")) {
        for (let i = 1; i <= 6; i++) {
    ...
    else if (viewer.urdf && viewer.urdf.includes("TriATHLETE")) {
        for (let i = 1; i <= 6; i++) {

            const offset = i * Math.PI / 3;
            const ratio = Math.max(0, Math.sin(time + offset));

            viewer.setJointValue(`HP${i}`, THREE.MathUtils.lerp(-60, 0, ratio) * DEG2RAD);
            viewer.setJointValue(`KP${i}`, THREE.MathUtils.lerp(-60, -180, ratio) * DEG2RAD);
            viewer.setJointValue(`AP${i}`, THREE.MathUtils.lerp(10, 120, ratio) * DEG2RAD);

            viewer.setJointValue(`TC${i}A`, THREE.MathUtils.lerp(0, 0.1, ratio));
            viewer.setJointValue(`TC${i}B`, THREE.MathUtils.lerp(0, 0.1, ratio));
...
const updateList = () => {
    ...
    document.querySelectorAll('#urdf-options li[urdf]').forEach(el => {
        ...
        // const robotName = el.textContent;//Indicar nuestro robot
        el.addEventListener('click', e => {

            const urdf = e.target.getAttribute('urdf');
            const color = e.target.getAttribute('color');

            /* if (robotName == 'UR5') {//Indicar nuestro robot
                viewer.up = '+Z';
            ...
            viewer.urdf = urdf;
            animToggle.classList.add('checked');//add
            setColor(color);
